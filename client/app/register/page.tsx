  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err && typeof err === 'object' && 'response' in err) {
        ...
        // Safely unpack zod errors
        if (Array.isArray(axiosError.response?.data?.error)) {
          setError(axiosError.response.data.error[0].message || 'Registration failed');
        } else {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center pt-16 px-4">
      ...
      <div className="glass p-8 rounded-2xl w-full max-w-md animate-fade-in shadow-2xl border-border">
        <h2 className="text-3xl font-bold mb-6 text-center">Join CineVault</h2>
